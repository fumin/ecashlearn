...
var (
	enumKind = reflect.ValueOf(Enum(0)).Kind()
)
...
func Deserialize(s interface{}, data []byte) error {
	...
	if v.Kind() != reflect.Ptr {
		return errors.Errorf("passed struct must be pointer")
	}
...
func read(r io.Reader, n int) ([]byte, error) {
	...
	l, err := r.Read(b)
	if l != n {
		return nil, errors.Errorf("failed to read required bytes")
	}
	if err != nil {
...
func deserialize(t reflect.Type, r io.Reader) (interface{}, error) {
	...
	case reflect.Float32:
		...
		if math.IsNaN(float64(f)) {
			return nil, errors.Errorf("NaN for float not allowed")
		}
		return f, nil
	...
	case reflect.Slice:
		...
		for i := 0; i < l; i++ {
			av, err := deserialize(t.Elem(), r)
			if err != nil {
				return nil, errors.Wrap(err, fmt.Sprintf("slice %d/%d", i, l))
			}
...
func deserializeComplexEnum(t reflect.Type, r io.Reader) (interface{}, error) {
	...
	enum := binary.LittleEndian.Uint32(tmp)
	v.Field(0).Set(reflect.ValueOf(Enum(enum)))
	// read enum field, if necessary
	if int(enum)+1 >= t.NumField() {
		return nil, errors.Errorf("complex enum too large")
	}
	fv, err := deserialize(t.Field(int(enum)+1).Type, r)
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("enum %d", int(enum)+1))
	}
...
func deserializeStruct(t reflect.Type, r io.Reader) (interface{}, error) {
	...
	if t.NumField() > 0 {
		...
		if firstField.Type.Kind() == enumKind &&
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if err != nil {
			return nil, errors.Wrap(err, fmt.Sprintf("%d %s", i, field.Name))
		}
...
func Serialize(s interface{}) ([]byte, error) {
	...
	if err := serialize(reflect.ValueOf(s), result); err != nil {
		return nil, errors.Wrap(err, "")
	}
...
func serializeComplexEnum(v reflect.Value, b io.Writer) error {
	...
	if err := binary.Write(b, binary.LittleEndian, enum); err != nil {
		return errors.Wrap(err, "")
	}
	// write enum field, if necessary
	if int(enum)+1 >= t.NumField() {
		return errors.Errorf("complex enum too large")
	}
...
func serializeUint128(v reflect.Value, b io.Writer) error {
	...
	buf := u.Bytes()
	if len(buf) > 16 {
		return errors.Errorf("big.Int too large for u128")
	}
...
func serialize(v reflect.Value, b io.Writer) error {
	...
	case reflect.Float32:
		...
		f := v.Float()
		if f == math.NaN() {
			return errors.Errorf("NaN float value")
		}
	...
	case reflect.String:
		tmp := make([]byte, 8)
		binary.LittleEndian.PutUint64(tmp, uint64(len(v.String())))
		_, err = b.Write(tmp)
		if err != nil {
			break
		}
